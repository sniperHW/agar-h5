...
static chk_acceptor *_chk_listen(chk_event_loop *loop,chk_sockaddr *addr,chk_acceptor_cb cb,chk_ud ud) {
	int32_t       fd,family;
	chk_acceptor *a = NULL;
	errno = 0;
...
chk_acceptor *chk_listen(chk_event_loop *loop,chk_sockaddr *addr,chk_acceptor_cb cb,chk_ud ud) {
	...
	return _chk_listen(loop,addr,cb,ud);
}

chk_acceptor *chk_ssl_listen(chk_event_loop *loop,chk_sockaddr *addr,chk_acceptor_cb cb,chk_ud ud) {
	if(NULL == loop || NULL == addr || NULL == cb) {
		CHK_SYSLOG(LOG_ERROR,"NULL == loop || NULL == addr || NULL == cb");		
		return NULL;
	}
